...
class Statement
{
    ...
    public function execute(array $params = [])
    {
        ...
        if ($this->bindings) {
            ...
            foreach ($this->bindings as $placeholder => $value) {
                ...
                if ($value !== null) {
                    ...
                    // SQLite-backed WordPress does not implement MySQL STR_TO_DATE/DATE
                    // conversion functions. When a legacy query still embeds one around a
                    // null-bound placeholder, replace that full expression with SQL NULL
                    // instead of allowing the database adapter to fail on it.
                    $patterns = [
                        '/STR_TO_DATE\s*\(\s*' . preg_quote($placeholder, '/') . '\s*,\s*[^)]*\)/i',
                        '/DATE\s*\(\s*' . preg_quote($placeholder, '/') . '\s*\)/i',
                    ];
        ...
        $query = $this->normalizeQueryForSqlite($query);
     ...
     */
    /**
     * Rewrite unsupported MySQL date conversion calls into SQLite-compatible equivalents
     * before the query is sent to the database.
     */
    private function normalizeQueryForSqlite($query)
    {
        $query = preg_replace_callback(
            '/STR_TO_DATE\s*\(\s*(?:\'([^\']*)\'|"([^"]*)")\s*,\s*(?:\'([^\']*)\'|"([^"]*)")\s*\)/i',
            function ($matches) {
                $value = $matches[1] !== '' ? $matches[1] : $matches[2];
                $format = strtolower($matches[3] !== '' ? $matches[3] : $matches[4]);

                if (strpos($format, '%h') !== false || strpos($format, '%i') !== false || strpos($format, '%s') !== false) {
                    return "time('" . str_replace("'", "''", $value) . "')";
                }

                if (strpos($format, '%d') !== false || strpos($format, '%m') !== false || strpos($format, '%y') !== false || strpos($format, '%Y') !== false) {
                    return "date('" . str_replace("'", "''", $value) . "')";
                }
