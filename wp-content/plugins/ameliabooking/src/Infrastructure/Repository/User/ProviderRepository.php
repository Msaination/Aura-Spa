...
class ProviderRepository extends UserRepository implements ProviderRepositoryInterface
{
    ...
    public function getOnSpecialDay()
    {
        ...
        $currentDateTime   = "datetime('" . $dateTimeNowString . "')";
        ...
        try {
            $statement = $this->connection->prepare(
                ...
                IF (
                    {$currentDateTime} >= datetime(sdt.startDate || ' 00:00:00') AND
                    {$currentDateTime} <= datetime(sdt.endDate || ' 00:00:00', '+1 day') AND
                    {$currentDateTime} >= datetime('{$currentDateString}' || ' ' || sdpt.startTime) AND
                    {$currentDateTime} <= datetime('{$currentDateString}' || ' ' || sdpt.endTime),
                    1,
                    0
                ) AS available
              FROM {$this->table} u
              ...
              WHERE u.type = :type AND
                date('{$currentDateString}') BETWEEN sdt.startDate AND sdt.endDate
              "
            );
