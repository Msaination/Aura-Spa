...
class AutoUpdateHook
{
    ...
    public static function checkUpdate($transient)
    {
        ...
        /** @var bool $activated */
        $activated = (bool) $settingsService->getSetting('activation', 'active');
        ...
        if (empty($purchaseCode) || empty($envatoTokenEmail)) {
            return $transient;
        }
    ...
    {
        if (empty($purchaseCode) || empty($envatoTokenEmail) || empty(AMELIA_STORE_API_URL)) {
            return false;
        }
        ...
        $info = maybe_unserialize($decoded->info);
