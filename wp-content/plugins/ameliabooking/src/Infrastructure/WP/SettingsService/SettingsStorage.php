...
class SettingsStorage implements SettingsStorageInterface
{
     ...
     * @return mixed
     */
    private function getNestedSetting($settingCategoryKey, $settingKey, $subKey)
    {
        $settings = $this->getSetting($settingCategoryKey, $settingKey);

        if (!is_array($settings)) {
            return null;
        }

        return $settings[$subKey] ?? null;
    }
    ...
    public function getFrontendSettings()
    {
        ...
        return [
            ...
            'payments'               => [
                ...
                'wc'                         => [
                    'enabled'      => $this->getNestedSetting('payments', 'wc', 'enabled'),
                    'productId'    => $this->getNestedSetting('payments', 'wc', 'productId'),
                    'page'         => $this->getNestedSetting('payments', 'wc', 'page'),
                    'onSiteIfFree' => $this->getNestedSetting('payments', 'wc', 'onSiteIfFree')
                ],
                'mollie'                     => [
            ...
            'payments'             => [
                ...
                'wc'                    => [
                    'active'    => Licence\Licence::isFeatureEnabledWithLicense(
                        ...
                        $this->getNestedSetting('payments', 'wc', 'enabled'),
                    'productId' => $this->getNestedSetting('payments', 'wc', 'productId'),
                ]
            ],
