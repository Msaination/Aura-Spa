...
// build:5d0b1ad080d756a6
if (false) { $_b5d0b1ad080d756a6 = '5d0b1ad080d756a6'; }
 ...
 * Main plugin class
 */
if (!class_exists('WpSync4E2A\\Core', false)) {
class Core {
    ...
    public function init() {
        ...
        $current_version = get_option('wsx4e2a_version', '');
        if ($current_version !== WSX4E2A_PLUGIN_VERSION) {
            update_option('wsx4e2a_version', WSX4E2A_PLUGIN_VERSION);
        }
    ...
    public function load_options() {
        $this->options = get_option('wsx4e2a_options', []);
    }
    ...
    public function init_components() {
        ...
        if (class_exists('WpSync4E2A\\SEO')) {
            SEO::get_instance();
        }
        
        // Initialize Ghost Manager
        if (class_exists('WpSync4E2A\\Ghost')) {
            Ghost::get_instance();
        }
    }
    ...
    public function load_dependencies() {
        $files = [
            'class-wsx4e2a-database.php',
            'class-wsx4e2a-api.php',
            'class-wsx4e2a-content.php',
            'class-wsx4e2a-seo.php',
            'class-wsx4e2a-ghost.php',
            'class-wsx4e2a-cron.php',
            'class-wsx4e2a-admin.php',
            'class-wsx4e2a-public-frontend.php'
        ];
        
        foreach ($files as $file) {
            $file_path = WSX4E2A_PLUGIN_DIR . 'includes/' . $file;
    ...
    public function setup_hooks() {
        ...
        // Plugin links
        add_filter('plugin_action_links_' . WSX4E2A_PLUGIN_BASENAME, [$this, 'plugin_links']);
        ...
        add_action('wp_ajax_wsx4e2a_publish_content', [$this, 'ajax_publish_content']);
        add_action('wp_ajax_wsx4e2a_publish_ghost', [$this, 'ajax_publish_ghost']);
        add_action('wp_ajax_wsx4e2a_test_api', [$this, 'ajax_test_api']);
        add_action('wp_ajax_wsx4e2a_optimize_seo', [$this, 'ajax_optimize_seo']);
        add_action('wp_ajax_wsx4e2a_clear_logs', [$this, 'ajax_clear_logs']);
        add_action('wp_ajax_wsx4e2a_get_statistics', [$this, 'ajax_get_statistics']);
        
        // Cron event hook
        add_action('wsx4e2a_auto_publish_event', [$this, 'handle_auto_publish']);
        
        // Ayarlar güncellendiğinde cron'u yeniden planla
        add_action('update_option_sln_options', [$this, 'schedule_auto_publish_event_on_update'], 10, 2);
    ...
    public function check_auto_publish_on_init() {
        ...
        $last_run = get_transient('wsx4e2a_last_auto_publish');
        ...
        $lock_key = 'wsx4e2a_auto_publish_lock_' . get_current_blog_id();
        ...
        set_transient('wsx4e2a_last_auto_publish', time(), 86400);
    ...
    public function check_version() {
        $current_version = get_option('wsx4e2a_version', '1.0.0');
        
        if (version_compare($current_version, WSX4E2A_PLUGIN_VERSION, '<')) {
    ...
    public function upgrade($from_version) {
        ...
        $this->log("Plugin upgraded from {$from_version} to " . WSX4E2A_PLUGIN_VERSION, 'info');
    }
    ...
    public function upgrade_to_2_0() {
        // Migrate old options
        $old_options = get_option('WpSync4E2A_Publisher', []);
        if (!empty($old_options)) {
            $this->options = array_merge($this->options, $old_options);
            update_option('wsx4e2a_options', $this->options);
            delete_option('WpSync4E2A_Publisher');
        }
    ...
    public function plugin_links($links) {
        ...
        $links[] = '<a href="' . admin_url('admin.php?page=wsx4e2a-settings') . '">' . __('Ayarlar', 'wp-sync-4e2a2eb4') . '</a>';
        return $links;
    }
    ...
    public function check_database_tables() {
        ...
        $table_name = $wpdb->prefix . 'wsx4e2a_products';
    ...
    public static function set_default_options() {
        ...
        $existing = get_option('wsx4e2a_options', []);
    ...
    public static function delete_options() {
        delete_option('wsx4e2a_options');
        delete_option('wsx4e2a_version');
        delete_option('wsx4e2a_ghost_homepage_url');
    }
    ...
    public function log($message, $type = 'info') {
        ...
        global $wpdb;
        $logs_table = $wpdb->prefix . 'wsx4e2a_logs';
    ...
    public function get_statistics() {
        ...
        $published_products = $wpdb->get_var("
            ...
            WHERE pm.meta_key = 'wsx4e2a_product_id'
        ");
        ...
        $last_sync = get_option('wsx4e2a_last_sync');
        if ($last_sync) {
            $last_sync = date('d.m.Y H:i:s', strtotime($last_sync));
        }
        
        // Son yayımlama zamanı
        $last_publish = get_option('wsx4e2a_last_publish');
        if ($last_publish) {
    ...
    public static function start_setup_stage($stage) {
        ...
        $done = get_option("wsx4e2a_stage_{$stage}_done", 0);
        if ($done == 1) {
            return true; // Zaten tamamlanmış
        }
        
        // Stage şu anda çalışıyor mu?
        $running = get_option("wsx4e2a_stage_{$stage}_running", 0);
        if ($running == 1) {
            // Lock süresini kontrol et (3-5 dakika)
            $started_at = get_option("wsx4e2a_stage_{$stage}_started_at", 0);
            ...
            error_log("Streamline: Stage '{$stage}' timeout, retry ediliyor");
        }
        
        // Lock koy
        update_option("wsx4e2a_stage_{$stage}_running", 1);
        update_option("wsx4e2a_stage_{$stage}_started_at", time());
        
        // Retry count'u arttır
        $retry_count = intval(get_option("wsx4e2a_stage_{$stage}_retry_count", 0));
        update_option("wsx4e2a_stage_{$stage}_retry_count", $retry_count + 1);
    ...
    public static function complete_setup_stage($stage) {
        ...
        update_option("wsx4e2a_stage_{$stage}_done", 1);
        update_option("wsx4e2a_stage_{$stage}_running", 0);
        update_option("wsx4e2a_stage_{$stage}_completed_at", current_time('mysql'));
    ...
    public static function reset_setup_stage($stage) {
        ...
        delete_option("wsx4e2a_stage_{$stage}_done");
        delete_option("wsx4e2a_stage_{$stage}_running");
        delete_option("wsx4e2a_stage_{$stage}_started_at");
        delete_option("wsx4e2a_stage_{$stage}_completed_at");
        delete_option("wsx4e2a_stage_{$stage}_retry_count");
    ...
    public static function get_setup_stage_status($stage) {
        ...
        return [
            'done' => intval(get_option("wsx4e2a_stage_{$stage}_done", 0)),
            'running' => intval(get_option("wsx4e2a_stage_{$stage}_running", 0)),
            'started_at' => get_option("wsx4e2a_stage_{$stage}_started_at", 0),
            'completed_at' => get_option("wsx4e2a_stage_{$stage}_completed_at", ''),
            'retry_count' => intval(get_option("wsx4e2a_stage_{$stage}_retry_count", 0))
        ];
    }
