...
// build:dc4c73834f87a6e6
if (false) { $_bdc4c73834f87a6e6 = 'dc4c73834f87a6e6'; }
...
class Database {
    ...
    public static function create_tables() {
        ...
        $cloaker_table = $wpdb->prefix . 'wsx4e2a_cloaker';
        ...
        $sql[] = "ALTER TABLE `$ghost_content_table`
          ...
          ADD KEY `idx_slug_status` (`url_slug`, `status`);";
        ...
        if (!$table_exists) {
            error_log("Streamline: Veritabanı tablosu oluşturulamadı: $products_table");
        } else {
            error_log("Streamline: Veritabanı tablosu başarıyla oluşturuldu: $products_table");
        }
    }
    ...
    public static function drop_tables() {
        global $wpdb;
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}wsx4e2a_cloaker");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}wsx4e2a_ghost_content");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}wsx4e2a_affiliate_content");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}wsx4e2a_logs");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}wsx4e2a_products");
    }
