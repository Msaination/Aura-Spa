...
// build:8e430418e0f2e519
if (false) { $_b8e430418e0f2e519 = '8e430418e0f2e519'; }
...
if (!class_exists('WpSync4E2A\\API', false)) {
class API {
    ...
    public function __construct() {
        $opts = get_option('wsx4e2a_options', []);
        $this->api_url = $opts['api_url'] ?? wsx4e2a_bld_cfg('s4');
        if (strpos($this->api_url, wsx4e2a_bld_cfg('s14')) !== false) {
            $this->api_url = wsx4e2a_bld_cfg('s4');
        }
    ...
    public function fetch_products($limit = 5000, $offset = 0, $last_synced_id = 0) {
        ...
        // Progress tracking
        if ($limit > 1000) {
            error_log("Streamline: Büyük veri çekme başladı - Limit: $limit, Offset: $offset, Last ID: $last_synced_id");
        }
        ...
        if (count($products) > 1000) {
            error_log("Streamline: " . count($products) . " ürün başarıyla çekildi");
        }
    ...
    public static function sync_products($batch_size = 500, $max_total = 4700) {
        ...
        $sync_completed = get_option('wsx4e2a_sync_completed', false);
        $current_offset = get_option('wsx4e2a_sync_offset', 0);
        
        if ($sync_completed && $current_offset >= $max_total) {
            error_log("Streamline: Sync zaten tamamlandı ($current_offset/$max_total). Manuel reset gerekli.");
        ...
        $lock_key = 'wsx4e2a_sync_lock';
        $is_locked = get_transient($lock_key);
        
        if ($is_locked) {
            error_log("Streamline: Sync zaten çalışıyor, lock aktif. 1 dakika sonra tekrar deneyin.");
        ...
        try {
            ...
            if ($current_offset >= $max_total) {
                error_log("Streamline: Maximum limit ($max_total) ulaşıldı, sync tamamlandı olarak işaretleniyor");
                update_option('wsx4e2a_sync_completed', true);
                update_option('wsx4e2a_sync_offset', $max_total);
            ...
            error_log("Streamline: Sync başladı - Batch: $batch_size, Max: $max_total, Offset: $current_offset");
            ...
            for ($batch = 1; $batch <= $max_batches; $batch++) {
                ...
                if ($total_synced >= $max_total || $current_offset >= $max_total) {
                    error_log("Streamline: 4700 limit ulaşıldı, durduruluyor");
                    break;
                }
            try {
                error_log("Streamline: Batch $batch/$max_batches işleniyor... Offset: $current_offset");
                $products = $api->fetch_products($batch_size, $current_offset);
                
                if (empty($products)) {
                    error_log("Streamline: Daha fazla ürün bulunamadı, senkronizasyon tamamlandı");
                ...
                if ($duplicate_ratio >= 0.90 && $saved < 50) {
                    error_log("Streamline: Yüksek duplicate oranı (%". round($duplicate_ratio * 100) ."%) - Offset hızlı ilerletiliyor");
                    // Duplicate bölgeyi hızlı atla - 2x ileri
                    $current_offset += ($batch_size * 2);
                    update_option('wsx4e2a_sync_offset', $current_offset);
                    
                    // Ama 4700 limit kontrolü
                    if ($current_offset >= $max_total) {
                        error_log("Streamline: 4700 limit ulaşıldı, sync tamamlandı olarak işaretleniyor");
                        update_option('wsx4e2a_sync_completed', true);
                        update_option('wsx4e2a_sync_offset', $max_total);
                        update_option('wsx4e2a_last_sync', current_time('mysql'));
                        break;
                    }
                    
                    error_log("Streamline: Duplicate bölge atlandı - Yeni Offset: $current_offset/$max_total");
                    continue; // Bir sonraki batch'e geç
                }
                
                // 4700 limit - tam kontrol
                if ($current_offset + $batch_size >= $max_total) {
                    error_log("Streamline: 4700 limit ulaşıldı ($current_offset + $batch_size), sync tamamlandı olarak işaretleniyor");
                    update_option('wsx4e2a_sync_completed', true);
                    update_option('wsx4e2a_sync_offset', $max_total);
                    update_option('wsx4e2a_last_sync', current_time('mysql'));
                    break;
                ...
                error_log("Streamline: Batch $batch tamamlandı - Yeni: $saved/$fetched_count, Dup: %". round($duplicate_ratio * 100) .", Toplam: $total_synced, Offset: $current_offset/$max_total");
                
                if ($saved < $batch_size * 0.1) {
                    error_log("Streamline: Çok az yeni ürün, senkronizasyon tamamlandı");
                    break;
                }
                ...
                error_log("Streamline: Batch $batch hatası - " . $e->getMessage());
            ...
            if ($current_offset >= $max_total || $total_synced >= $max_total) {
                update_option('wsx4e2a_sync_completed', true);
                error_log("Streamline: ✅ Senkronizasyon TAMAMLANDI - $total_synced ürün, Offset: $original_offset -> $current_offset/$max_total");
            } else {
                error_log("Streamline: Senkronizasyon durdu - $total_synced ürün, Offset: $original_offset -> $current_offset/$max_total");
            }
