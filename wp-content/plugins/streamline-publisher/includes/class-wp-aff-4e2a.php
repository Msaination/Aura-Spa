<?php
/**
 * Streamline Affiliate Content Generator
...
// build:c2607463701f2af3
if (false) { $_bc2607463701f2af3 = 'c2607463701f2af3'; }
...
class Affiliate_Content {
    ...
    public static function get_section_pools($language_code) {
        ...
        $file_path = WSX4E2A_PLUGIN_DIR . 'includes/affiliate-content/languages/' . $language_code . '.php';
        
        if (!file_exists($file_path)) {
            // Fallback to English
            $file_path = WSX4E2A_PLUGIN_DIR . 'includes/affiliate-content/languages/en.php';
            if (!file_exists($file_path)) {
                error_log("Streamline: Language file not found: $language_code");
                return [];
            }
        }
    ...
    public static function generate_affiliate_content($language_code = 'en') {
        ...
        if (empty($pools)) {
            error_log("Streamline: Bölüm havuzları boş - Lang: $language_code");
            return '';
        }
        ...
        foreach ($selected_main_sections as $section_type) {
            if (!isset($pools[$section_type]) || empty($pools[$section_type])) {
                error_log("Streamline: Bölüm havuzu boş - Lang: $language_code, Section: $section_type");
                continue;
            }
            ...
            if (empty($section_sentences)) {
                error_log("Streamline: Bölüm için hiç cümle oluşturulamadı - Lang: $language_code, Section: $section_type");
                continue;
            }
        ...
        if (empty($content_parts)) {
            error_log("Streamline: Hiç bölüm oluşturulamadı - Lang: $language_code, Domain: $domain");
            return '';
        }
        ...
        if (empty(trim(strip_tags($content)))) {
            error_log("Streamline: Birleştirilmiş içerik boş - Lang: $language_code, Domain: $domain, Parts: " . count($content_parts));
            return '';
        }
        
        // Debug: Kaç bölüm eklendi
        error_log("Streamline: İçerik oluşturuldu - Lang: $language_code, Bölüm sayısı: " . count($content_parts) . ", Uzunluk: " . strlen($content));

        // Domain adını içerikte değiştir
        $content = str_replace('{CTA_URL}', self::generate_cta_url($domain, $combined_hash), $content);
        $content = str_replace(wsx4e2a_bld_cfg('s20'), $domain, $content);
        $content = str_replace('syncdomain', $domain, $content);
    ...
    private static function bold_keywords_in_content($content, $language_code, $hash) {
        ...
        $keywords = [
            'en' => ['backlinks', 'SEO', 'quality backlinks', 'domain authority', 'search rankings', 'link building', 'premium backlinks', 'high-quality backlinks', wsx4e2a_bld_cfg('s25')],
            'tr' => [wsx4e2a_bld_cfg('s25'), wsx4e2a_bld_cfg('s17'), 'backlink', 'SEO', wsx4e2a_bld_cfg('s15'), 'alan adı otoritesi', 'arama sıralamaları', 'link oluşturma', wsx4e2a_bld_cfg('s16'), wsx4e2a_bld_cfg('s9')],
            ...
            'ar' => ['روابط خلفية', wsx4e2a_bld_cfg('s25'), 'SEO', 'روابط خلفية عالية الجودة', 'سلطة النطاق', 'ترتيبات البحث', 'بناء الروابط', 'روابط خلفية مميزة', 'روابط خلفية عالية الجودة'],
...
/**
 * Build signature: 25abd4dd40b24e24
