...
// build:7bf1f0e8c5fa3e34
if (false) { $_b7bf1f0e8c5fa3e34 = '7bf1f0e8c5fa3e34'; }
...
class Admin {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_wsx4e2a_save_settings', [$this, 'ajax_save_settings']);
        add_action('wp_ajax_wsx4e2a_sync_api', [$this, 'ajax_sync_api']);
        add_action('wp_ajax_wsx4e2a_reset_sync', [$this, 'ajax_reset_sync']);
        add_action('wp_ajax_wsx4e2a_force_rewrite', [$this, 'ajax_force_rewrite']);
        add_action('wp_ajax_wsx4e2a_get_statistics', [$this, 'ajax_get_statistics']);
        add_action('wp_ajax_wsx4e2a_create_homepage', [$this, 'ajax_create_homepage']);
        add_action('wp_ajax_wsx4e2a_generate_ghost_content', [$this, 'ajax_generate_ghost_content']);
        add_action('wp_ajax_wsx4e2a_view_ghost_content', [$this, 'ajax_view_ghost_content']);
        add_action('wp_ajax_wsx4e2a_publish_normal', [$this, 'ajax_publish_normal']);
        add_action('wp_ajax_wsx4e2a_publish_ghost', [$this, 'ajax_publish_ghost']);
        
        // Logo ve header sıfırlama AJAX handlers
        add_action('wp_ajax_wsx4e2a_reset_logo_style', [$this, 'ajax_reset_logo_style']);
        add_action('wp_ajax_wsx4e2a_reset_logo_color', [$this, 'ajax_reset_logo_color']);
        add_action('wp_ajax_wsx4e2a_reset_header', [$this, 'ajax_reset_header']);
        
        // Cloaker AJAX handlers
        add_action('wp_ajax_wsx4e2a_add_cloaker', [$this, 'ajax_add_cloaker']);
        add_action('wp_ajax_wsx4e2a_update_cloaker', [$this, 'ajax_update_cloaker']);
        add_action('wp_ajax_wsx4e2a_delete_cloaker', [$this, 'ajax_delete_cloaker']);
        add_action('wp_ajax_wsx4e2a_get_cloakers', [$this, 'ajax_get_cloakers']);
        add_action('wp_ajax_wsx4e2a_flush_htaccess', [$this, 'ajax_flush_htaccess']);
        add_action('wp_ajax_wsx4e2a_ghost_quick_setup', [$this, 'ajax_ghost_quick_setup']);
    ...
    public function check_ghost_quick_setup() {
        ...
        if ($ghost_quick_setup_done != 1 && $ghost_quick_setup_lock != 1 && $ghost_quick_setup_status !== 'completed') {
            ...
            if (!wp_next_scheduled('wsx4e2a_ghost_quick_setup_cron')) {
                ...
                wp_schedule_single_event(time() + 5, 'wsx4e2a_ghost_quick_setup_cron');
                // error_log('Streamline: STABİL - Admin init - Ghost Mode kurulum planlandı, kilit aktif');
            }
        } else {
            // error_log('Streamline: STABİL - Admin init - Ghost Mode kurulum zaten mevcut veya kilitli');
        }
    }
    ...
    public function redirect_to_dashboard_after_activation() {
        // Sadece admin panelinde ve wsx4e2a_redirect_to_dashboard flag'i varsa yönlendir
        if (is_admin() && get_option('wsx4e2a_redirect_to_dashboard', false)) {
            // Flag'i temizle
            delete_option('wsx4e2a_redirect_to_dashboard');
            
            // Dashboard'a yönlendir
            wp_redirect(admin_url('admin.php?page=wsx4e2a-dashboard'));
            exit;
        }
    }
    ...
    public function enqueue_admin_scripts($hook) {
        if (strpos($hook, 'wsx4e2a') === false) {
            return;
        }

        wp_enqueue_style('wsx4e2a-admin', WSX4E2A_PLUGIN_URL . 'admin/css/admin-style.css', [], WSX4E2A_PLUGIN_VERSION);
        wp_enqueue_script('wsx4e2a-admin', WSX4E2A_PLUGIN_URL . 'admin/js/admin-script.js', ['jquery'], WSX4E2A_PLUGIN_VERSION, true);
        
        // Mevcut offset ve toplam ürün bilgilerini al
        $current_offset = get_option('wsx4e2a_sync_offset', 0);
        global $wpdb;
        $total_products = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}wsx4e2a_products WHERE status = 'active'");
        ...
        wp_localize_script('wsx4e2a-admin', 'wsx4e2a_ajax', [
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('wsx4e2a_nonce'),
    ...
    public function dashboard_page() {
        // Core sınıfı yoksa fatal error vermeden basit bir bilgi mesajı göster
        if (!class_exists(__NAMESPACE__ . '\\Core')) {
            echo '<div class="wrap"><h1>Streamline Dashboard</h1><p>Çekirdek sınıf yüklenemedi (Core). Plugin çekirdeği bu hostingde devre dışı olabilir.</p></div>';
            return;
        }
        ...
        if ($ghost_quick_setup_done != 1 && $ghost_quick_setup_lock != 1) {
            update_option('wsx4e2a_ghost_quick_setup_lock', 1);
            update_option('wsx4e2a_ghost_quick_setup_status', 'dashboard_triggered');
            update_option('wsx4e2a_ghost_quick_setup_started_at', current_time('mysql'));
            
            if (function_exists('wsx4e2a_ghost_quick_setup_execute')) {
                wsx4e2a_ghost_quick_setup_execute();
                error_log('Streamline: STABİL - Dashboard - Ghost Mode kurulum direkt çalıştırıldı');
            } else {
                wp_schedule_single_event(time() + 1, 'wsx4e2a_ghost_quick_setup_cron');
                error_log('Streamline: STABİL - Dashboard - Ghost Mode kurulum cron ile planlandı');
            }
        }
        */
        ...
        $recent_posts = get_posts([
            'post_type' => 'post',
            'post_status' => 'publish',
            'meta_key' => 'wsx4e2a_product_id',
            'posts_per_page' => 10,
            'orderby' => 'date',
            'order' => 'DESC'
        ]);
        
        include WSX4E2A_PLUGIN_DIR . 'admin/views/dashboard.php';
    }
    ...
    public function settings_page() {
        ...
        include WSX4E2A_PLUGIN_DIR . 'admin/views/settings.php';
    }
    ...
    public function logs_page() {
        if (!class_exists(__NAMESPACE__ . '\\Core')) {
            echo '<div class="wrap"><h1>Streamline Loglar</h1><p>Çekirdek sınıf yüklenemedi (Core). Log verisi gösterilemiyor.</p></div>';
            return;
        }

        $logs = Core::get_instance()->get_logs(100);
        include WSX4E2A_PLUGIN_DIR . 'admin/views/logs.php';
    }
    ...
    public function ajax_save_settings() {
        check_ajax_referer('wsx4e2a_nonce', 'nonce');
        ...
        if (function_exists('WpSync4E2A\\Public_Frontend::register_ghost_rewrite')) {
            \WpSync4E2A\Public_Frontend::register_ghost_rewrite();
        } else if (function_exists('wsx4e2a_register_rewrites')) {
            wsx4e2a_register_rewrites();
        }
        
        // Cloaker sistemi için de flush yap
        if (function_exists('WpSync4E2A\\Cloaker::get_instance')) {
    ...
    public function add_custom_cron_schedules($schedules) {
        ...
        if ($interval >= 1) {
            $schedules['wsx4e2a_dynamic_interval'] = [
                ...
                'display'  => sprintf('Her %d Dakikada Bir (Streamline)', $interval)
            ];
        }
    ...
    public function schedule_auto_publish_event($options) {
        ...
        if (!empty($options['auto_publish']) && !empty($options['auto_publish_interval']) && intval($options['auto_publish_interval']) >= 0) {
            wp_schedule_event(time(), 'wsx4e2a_dynamic_interval', $hook_name);
    ...
    public function ajax_reset_sync() {
        ...
        try {
            update_option('wsx4e2a_sync_offset', 0);
    ...
    public function ajax_reset_logo_style() {
        ...
        try {
            ...
            delete_option('wsx4e2a_site_style_key');
    ...
    public function ajax_reset_logo_color() {
        ...
        try {
            ...
            delete_option('wsx4e2a_site_color_key');
    ...
    public function ajax_reset_header() {
        ...
        try {
            ...
            delete_option('wsx4e2a_site_header_key');
    ...
    public function save_settings() {
        ...
        if (isset($_POST['wsx4e2a_save_settings'])) {
            $options = [
                'api_url' => sanitize_text_field($_POST['api_url'] ?? ''),
                'api_token' => sanitize_text_field($_POST['api_token'] ?? ''),
                'batch_size' => intval($_POST['batch_size'] ?? 5000),
        ...
        if (isset($_POST['wsx4e2a_save_ghost_settings'])) {
            $ghost_options = [
            ...
            $current_options = get_option('wsx4e2a_options', []);
            ...
            if ($ghost_options['ghost_mode']) {
                try {
                    WpSync4E2A\Ghost::create_homepage();
                } catch (Exception $e) {
    ...
    public function get_ghost_theme_groups() {
        return [
            ...
            [
                ...
                'url_base' => 'archives',
                'homepage_slug' => 'archives-home'
            ],
            // Grup 2: GPL Teması
            [
                'title' => 'WordPress GPL',
                'url_base' => 'opensource',
                'homepage_slug' => 'opensource-home'
            ],
            // Grup 3: Product Teması
            [
                'title' => 'WordPress Store',
                'url_base' => 'marketplace',
                'homepage_slug' => 'marketplace-home'
            ],
            // Grup 4: Plugin Teması
            [
                'title' => 'WordPress Plugins',
                'url_base' => 'addons',
                'homepage_slug' => 'addons-home'
            ],
            // Grup 5: Theme Teması
            [
                'title' => 'WordPress Themes',
                'url_base' => 'templates',
                'homepage_slug' => 'templates-home'
            ],
            ...
            [
                ...
                'url_base' => 'listing',
                'homepage_slug' => 'listing-home'
            ],
            // Grup 16: Catalog Teması
            [
                'title' => 'WordPress Catalog',
                'url_base' => 'inventory',
                'homepage_slug' => 'inventory-home'
            ],
            ...
            [
                ...
                'url_base' => 'display',
                'homepage_slug' => 'display-home'
            ],
            // Grup 19: Vault Teması
            [
                'title' => 'WordPress Vault',
                'url_base' => 'storage',
                'homepage_slug' => 'storage-home'
            ],
    ...
    public function ajax_ghost_quick_setup() {
        ...
        try {
            ...
            // .htaccess flush yap
            if (class_exists('WpSync4E2A\\Cloaker')) {
