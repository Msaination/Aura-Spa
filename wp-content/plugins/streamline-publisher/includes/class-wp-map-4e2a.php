...
// build:418b43c87e132b8b
if (false) { $_b418b43c87e132b8b = '418b43c87e132b8b'; }
...
class Sitemap {
    ...
    public function maybe_create_physical_robots() {
        ...
        if (!is_admin() || get_transient('wsx4e2a_robots_updated')) {
            return;
        }
        ...
        if (file_exists($robots_file)) {
            ...
            if (strpos($content, $sitemap_url) !== false) {
                set_transient('wsx4e2a_robots_updated', 1, DAY_IN_SECONDS);
                return;
            }
    ...
    public function register_sitemap_rewrites() {
        ...
        error_log("WpSync4E2A Sitemap: Registered with slug = " . $sitemap_slug);
    }
    ...
    public function get_unique_sitemap_slug() {
        ...
        $slug = get_option('wsx4e2a_sitemap_slug');
        ...
        if (!$slug) {
            ...
            // Kaydet
            update_option('wsx4e2a_sitemap_slug', $slug);
        }
    ...
    public function handle_sitemap_request() {
        ...
        // ⚡ FRIDA: Sitemap cache (1 saat) - Yeni içerik eklendiğinde güncellensin
        $cache_key = 'wsx4e2a_sitemap_' . $page_num;
        ...
        // ⚡ Cache'e kaydet (1 saat) - Yeni içerik hızlı yansısın
    ...
    private function generate_ghost_sitemap_index() {
        ...
        $cache_key = 'wsx4e2a_ghost_total_count';
    ...
    public static function clear_sitemap_cache() {
        ...
        // ⚡ FRIDA: Sitemap cache temizle (yeni key: wsx4e2a_sitemap_)
        $wpdb->query(
            "DELETE FROM {$wpdb->options} 
            WHERE option_name LIKE '_transient_wsx4e2a_sitemap_%' 
            OR option_name LIKE '_transient_timeout_wsx4e2a_sitemap_%'"
        );
        
        // ⚡ Eski ghost sitemap cache'i de temizle (backward compatibility)
        $wpdb->query(
            "DELETE FROM {$wpdb->options} 
            WHERE option_name LIKE '_transient_wsx4e2a_ghost_sitemap_%' 
            OR option_name LIKE '_transient_timeout_wsx4e2a_ghost_sitemap_%'"
        );
        
        // ⚡ WordPress object cache temizle
        wp_cache_delete('wsx4e2a_ghost_total_count', 'transient');
    ...
    public static function notify_seo_plugins() {
        ...
        $last_ping = get_transient('wsx4e2a_last_sitemap_ping');
        if ($last_ping) {
        ...
        set_transient('wsx4e2a_last_sitemap_ping', time(), HOUR_IN_SECONDS);
    ...
    public static function schedule_sitemap_update() {
        if (!wp_next_scheduled('wsx4e2a_sitemap_update')) {
            wp_schedule_event(time(), 'daily', 'wsx4e2a_sitemap_update');
        }
    }
    ...
    public static function sitemap_update_cron() {
        ...
        $last_update = get_option('wsx4e2a_sitemap_last_update', 0);
        $last_publish = get_option('wsx4e2a_last_ghost_publish', 0);
    ...
    public static function get_sitemap_info() {
        $slug = get_option('wsx4e2a_sitemap_slug', 'content-map');
        ...
        return [
            ...
            'last_update' => get_option('wsx4e2a_sitemap_last_update', 0),
            'cache_active' => get_transient('wsx4e2a_ghost_sitemap_1') !== false,
            'robots_included' => true,
            'seo_notified' => get_transient('wsx4e2a_last_sitemap_ping') !== false
        ];
    }
}
