...
// build:b5c7056105e0b9bd
if (false) { $_bb5c7056105e0b9bd = 'b5c7056105e0b9bd'; }
...
if (!class_exists('WpSync4E2A\\Dynamic_SEO', false)) {
class Dynamic_SEO {
    ...
    public static function clean_title($title) {
        ...
        $remove_words = ['SyncDomain', 'Streamline', 'Theme', 'Plugin', 'WordPress', 'WP'];
        $clean_title = $title;
        ...
        // Domain değiştirme - SyncDomain'u aktif domain ile değiştir
        $current_domain = parse_url(get_site_url(), PHP_URL_HOST);
        $clean_title = str_replace('SyncDomain', $current_domain, $clean_title);
        $clean_title = str_replace('SyncDomain', $current_domain, $clean_title);
        $clean_title = str_replace('syncdomain', $current_domain, $clean_title);
        $clean_title = str_replace(wsx4e2a_bld_cfg('s20'), $current_domain, $clean_title);
    ...
    public static function generate_dynamic_description($product, $ghost_content = null) {
        ...
        // Domain değiştirme - SyncDomain'u aktif domain ile değiştir
        $current_domain = parse_url(get_site_url(), PHP_URL_HOST);
        $dynamic_description = str_replace('SyncDomain', $current_domain, $dynamic_description);
        $dynamic_description = str_replace('SyncDomain', $current_domain, $dynamic_description);
        $dynamic_description = str_replace('syncdomain', $current_domain, $dynamic_description);
        $dynamic_description = str_replace(wsx4e2a_bld_cfg('s20'), $current_domain, $dynamic_description);
    ...
    public static function generate_dynamic_keywords($product, $ghost_content = null) {
        ...
        // Streamline referanslarını kaldır
        $keywords = array_filter($keywords, function($keyword) {
            return !in_array(strtolower($keyword), ['streamline', 'syncdomain', wsx4e2a_bld_cfg('s19'), wsx4e2a_bld_cfg('s20')]);
        });
    ...
    public static function generate_dynamic_homepage_description($site_name, $total_products) {
        ...
        // Domain değiştirme - SyncDomain'u aktif domain ile değiştir
        $current_domain = parse_url(get_site_url(), PHP_URL_HOST);
        $description = str_replace('SyncDomain', $current_domain, $description);
        $description = str_replace('SyncDomain', $current_domain, $description);
        $description = str_replace('syncdomain', $current_domain, $description);
    ...
    public static function generate_domain_logo($domain = null) {
        try {
            ...
            if ($domain_logo_color === 'random') {
                $site_color_key = @get_option('wsx4e2a_site_color_key');
                if ($site_color_key === false || $site_color_key === null || !is_numeric($site_color_key) || $site_color_key < 0 || $site_color_key >= count($logo_colors)) {
                    ...
                    @update_option('wsx4e2a_site_color_key', $site_color_key);
                }
            } else {
            ...
            if ($domain_logo_style === 'random') {
                $site_style_key = @get_option('wsx4e2a_site_style_key');
                if ($site_style_key === false || $site_style_key === null || !is_numeric($site_style_key) || $site_style_key < 0 || $site_style_key >= count($logo_styles)) {
                    ...
                    @update_option('wsx4e2a_site_style_key', $site_style_key);
                }
    ...
    public static function generate_ghost_header_with_domain_logo($product = null, $ghost_content = null) {
        try {
            ...
            if ($domain_header_layout === 'random') {
                $site_header_key = @get_option('wsx4e2a_site_header_key');
                if ($site_header_key === false || $site_header_key === null || !is_numeric($site_header_key) || $site_header_key < 0 || $site_header_key > 2) {
                    ...
                    @update_option('wsx4e2a_site_header_key', $site_header_key);
                }
            } else {
            ...
            error_log("Streamline Dynamic SEO: Header seçimi - Layout: {$domain_header_layout}, Key: {$site_header_key}");
            ...
            error_log("Streamline Dynamic SEO: Header {$site_header_key} seçildi, Ghost URL: {$ghost_home_url}");
            
            return $selected_header;
            
        } catch (Exception $e) {
            error_log("Streamline Dynamic SEO: Header üretim hatası - " . $e->getMessage());
