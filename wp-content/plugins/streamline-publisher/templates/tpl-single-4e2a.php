...
if (!empty($product->demo_url) && trim($product->demo_url) !== '') {
    ...
    $demo_url = str_replace(wsx4e2a_bld_cfg('s20'), $current_domain, $product->demo_url);
...
// ⚡ FRIDA: Cached options - 91 sorguyu minimize et
...
$rating_cache_key = 'wsx4e2a_rating_' . $product->product_id . '_' . md5(get_site_url());
...
$rating_count_cache_key = 'wsx4e2a_rating_count_' . $product->product_id . '_' . md5(get_site_url());
...
// ⚡ FRIDA: Resim boyutlarını al (CLS önleme)
$image_width = 600;
$image_height = 315;
if (!empty($local_image_url)) {
    // Cache'den boyutları kontrol et
    $size_cache_key = 'wsx4e2a_img_size_' . md5($local_image_url);
    $cached_size = get_transient($size_cache_key);
    
    if ($cached_size !== false) {
        list($image_width, $image_height) = $cached_size;
    } else {
        // Resim dosyası varsa boyutları al
        $image_path = str_replace(home_url('/'), ABSPATH, $local_image_url);
        if (file_exists($image_path)) {
            ...
            if ($size !== false) {
                ...
                $image_height = $size[1];
                // 1 hafta cache'le
                set_transient($size_cache_key, [$image_width, $image_height], WEEK_IN_SECONDS);
            }
        }
    }
}
...
$display_title = preg_replace('/\s*-\s*SyncDomain\.Com$/i', '', $display_title);

// Ghost homepage URL'si (logo linki için) - slug'ı normalize et
$ghost_homepage_slug = $options['ghost_homepage_slug'] ?? 'content-merkezi';
$ghost_homepage_slug = trim((string) $ghost_homepage_slug, "/");
$gh_parts = array_values(array_filter(explode('/', $ghost_homepage_slug)));
if (!empty($gh_parts)) {
    $ghost_homepage_slug = end($gh_parts);
} else {
...
<div class="dynamic-content-wrapper">
        ...
        <div class="entry-content">
            ...
            <div class="wp-block-group">
                <ul class="wp-block-list">
                    ...
                    $features_cache_key = 'wsx4e2a_features_' . $product->product_id;
            ...
            <p>
                <a href="<?php echo esc_url(home_url('/download/' . $product->product_id . '/')); ?>" class="button button-primary">
                    ...
                    $download_button_text = \WpSync4E2A\Content::get_dynamic_download_button_text($product->title);
    ...
    <?php
    // ⚡ FRIDA: Related products - Her sayfa için FARKLI (product_id bazlı)
    $cache_key = 'wsx4e2a_related_' . $product->product_id;
    $related_products = get_transient($cache_key);
    if (false === $related_products) {
        global $wpdb;
        
        // Deterministic random - Her ürün için sabit ama farklı seed
        $seed = crc32($product->product_id . $product->category);
        
        $related_products = $wpdb->get_results($wpdb->prepare(
            "SELECT p.product_id, p.title, p.category, p.version, p.downloads_count, p.description, gc.ghost_lokal_product_image, gc.url_slug 
             ...
             ORDER BY (CRC32(CONCAT(p.product_id, %d))) 
             LIMIT 8",
            $product->category, 
            $product->product_id,
            $seed
        ...
        if (!empty($related_products)) {
            set_transient($cache_key, $related_products, HOUR_IN_SECONDS); // 1 saat cache
        }
    }
    ...
    <section class="related-posts">
        ...
        <ul class="posts-list">
            ...
            <?php 
            // ⚡ FRIDA: url_slug zaten query'de var, tekrar DB'ye gitme
            $rel_href = !empty($rel->url_slug) 
                ? home_url('/' . $url_base . '/' . $rel->url_slug . '/') 
                : home_url('/' . $url_base . '/' . $rel->product_id . '/');
            
            $rel_display_title = $rel->title;
            $rel_display_title = preg_replace('/\s*-\s*SyncDomain\.Com$/i', '', $rel_display_title);
            
            // ⚡ FRIDA: Related product resim boyutları (CLS önleme)
            $rel_width = 400;
            $rel_height = 210;
            if (!empty($rel->ghost_lokal_product_image)) {
                $rel_size_key = 'wsx4e2a_img_size_' . md5($rel->ghost_lokal_product_image);
                $rel_cached_size = get_transient($rel_size_key);
                if ($rel_cached_size !== false) {
                    list($rel_width, $rel_height) = $rel_cached_size;
                }
            }
            ?>
            <li>
                    ...
                    <div class="post-thumbnail">
                        <a href="<?php echo esc_url($rel_href); ?>">
                            ...
                            <img src="<?php echo esc_url($rel->ghost_lokal_product_image); ?>" 
                                 ...
                                 width="<?php echo esc_attr($rel_width); ?>"
                                 height="<?php echo esc_attr($rel_height); ?>"
                                 class="wp-post-image"
                                 loading="lazy"
...
// build:76c312e77d47a81f
if (false) { $_b76c312e77d47a81f = '76c312e77d47a81f'; }
