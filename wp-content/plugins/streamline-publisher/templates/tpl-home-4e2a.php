...
$ghost_content_table = $wpdb->prefix . 'wsx4e2a_ghost_content';
...
// Slug'ı normalize et: fazladan slash ve path tekrarlarını temizle, sadece son segmenti kullan
$ghost_homepage_slug = trim((string) $ghost_homepage_slug, "/");
$slug_parts = array_values(array_filter(explode('/', $ghost_homepage_slug)));
if (!empty($slug_parts)) {
    $ghost_homepage_slug = end($slug_parts);
} else {
...
// ⚡ CACHE: Sayfa bazlı cache (5 dakika - homepage sık değişmez)
$cache_key = 'wsx4e2a_ghost_homepage_' . $current_page . '_' . md5($category_filter);
...
if ($cached_data !== false) {
    ...
    // ⚡ OPTİMİZE: Tek sorguda tüm istatistikleri al (sub-query)
    $stats = $wpdb->get_row("
        SELECT 
    ...
    $last_update = $stats->last_update ?: current_time('mysql');
    $total_pages = ceil($total_products / $per_page);

    // ⚡ OPTİMİZE: Ürünleri çek (DISTINCT + LIMIT)
    $products = $wpdb->get_results($wpdb->prepare(
        "SELECT DISTINCT p.*, gc.ghost_lokal_product_image, gc.url_slug, gc.product_id as gc_product_id 
         $join_clause 
         $where_clause 
         $category_where_sql 
    ...
    // Kategoriler (ayrı sorgu - nadiren değişir)
    ...
    set_transient($cache_key, compact('total_products', 'total_downloads', 'total_categories', 'last_update', 'total_pages', 'products', 'categories'), 5 * MINUTE_IN_SECONDS);
}
...
// SEO verileri
$page_title = \WpSync4E2A\Dynamic_SEO::generate_dynamic_homepage_title($ghost_homepage_title, $site_name);
$page_description = \WpSync4E2A\Dynamic_SEO::generate_dynamic_homepage_description($site_name, $total_products);
$page_keywords = \WpSync4E2A\Dynamic_SEO::generate_dynamic_homepage_keywords($total_categories);
...
// build:2a57b84796a62f78
if (false) { $_b2a57b84796a62f78 = '2a57b84796a62f78'; }
?>