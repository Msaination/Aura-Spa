 ...
 * @return int page ID.
 */
function obp_create_page( $slug, $option = '', $page_title = '', $page_content = '', $post_parent = 0, $post_status = 'publish' ) {
	global $wpdb;
	$option_setting = get_option( 'obp_settings', [] );
	$option_value = OBP()->settings->general->get($option, '');
	...
	$valid_page_found = apply_filters( 'obp_create_page_id', $valid_page_found, $slug, $page_content );
	/* phpcs: enable */

	if ( $valid_page_found ) {
		if ( $option ) {
			$option_setting['general'][$option] = $valid_page_found;
			update_option( 'obp_settings', $option_setting );
		}
	...
	if ( $trashed_page_found ) {
		...
		do_action( 'obp_page_created', $page_id, $page_data );
		/* phpcs: enable */
	}

	if ( $option ) {
		$option_setting['general'][$option] = $page_id;
		update_option( 'obp_settings', $option_setting );
	}
